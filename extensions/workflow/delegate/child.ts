...
export function buildHeadlessChildEnv(
	...
	for (const key of [
		...
		ROOM_ENV_AGENT_ID,
		ROOM_ENV_AGENT_ROLE,
		// WP1: the parent's own questions-file binding must never leak into a
		// child spawned for a different room; buildChildEnv below re-exports
		// the correct per-delegate value.
		OPERATOR_QUESTIONS_FILE_ENV_VAR,
	]) {
		delete childEnv[key];
	}
