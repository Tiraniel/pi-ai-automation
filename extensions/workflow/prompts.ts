...
2. For non-trivial changes, run the contract-first planning pipeline (below) as Brain-owned planning before delegating to coder. If task markers/config/user request deep planning, run planning-only deep-planning first with \`workflow_deep_plan\` (pass \`force:true\` if config default is disabled), then synthesize options/risks from the room transcript before step 3.
3. Send coder a self-contained implementation task with relevant files, constraints, expected checks, and the concrete Brain-authored block plan from step 2.
...
- Deep-planning handoff: for complex architecture-risk tasks, run \`workflow_deep_plan\` before coder delegation when a \`deep_planning\` marker (required/auto) requires it, explicit user request requires it, or Brain judges it valuable. Marker-driven required/auto opt-in should pass \`force:true\` if config default is disabled. Planner delegates are planning-only and must not edit files or run edit/write/bash. Brain must synthesize planner options/risks and then proceed with normal planning → implementation delegation.
