...
// WP1: single owner of the operator-escalation wording. Interpolated into
// BRAIN_INSTRUCTIONS below and into the default planner instructions
// (extensions/workflow/defaults.ts) — never restate it elsewhere.
export const OPERATOR_ESCALATION_RULE = `Operator escalation: when an ambiguity, unsafe assumption, or missing decision blocks progress, do not guess and do not silently stall waiting for chat input — record the question durably with the workflow_ask_operator tool (blocking=true only when work cannot safely proceed; include options and a recommendedDefault). Unanswered blocking questions gate prd_ready_for_sprint, task finalization, and AFK ship delivery until answered via workflow_answer_question.`;
