...
- **WP3 PRD-contract traceability.** Matrix entries may carry optional `criterionId` (`AC<N>`), `covers` (PRD contract ids `B<N>` / `X<N>`), and `negative: true` (a negative-scenario row proving a forbidden behavior does NOT happen). Malformed ids are rejected at normalization, never silently dropped. When the active planning room carries a structural `prd.json` (see `extensions/workflow/planning-prd-contract.ts`), the ready hard-lock additionally requires every forbidden behavior `X<N>` to be covered by at least one `negative: true` row (`forbidden_behavior_uncovered` otherwise). Design note: negative scenarios are expressed as the optional `negative` field rather than a new `criterionKind` — the field is additive and touches only the coverage validator, whereas widening the `criterionKind` closed set would ripple through every criterionKind consumer (prompt-only rules, reviewer role derivation, docs).
